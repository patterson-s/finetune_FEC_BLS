...
def process_unique_occupations_in_batches(unique_occupations: List[str], batch_size: int, temp_dir: Path) -> pd.DataFrame:
    """Process unique occupations in batches in reverse order and save progress incrementally."""
    ...
    with tqdm(total=num_batches, desc="Processing batches (Reverse)", unit="batch") as pbar:
        for i in range(len(unique_occupations) - batch_size, -1, -batch_size):
...
def main():
    ...
    # Process unique occupations in batches in reverse order
